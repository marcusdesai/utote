...
use std::mem;
use std::slice::{Iter, IterMut, Chunks, from_raw_parts_mut, ChunksMut};
...
impl<N: Copy, const SIZE: usize> Multiset2<N, SIZE> {
    ...
    #[inline]
    fn _zip_map_chunks_remainder<F, const C: usize>(&self, other: &Self, mut f: F) -> Self
        where
            N: SmallNumConsts,
            F: FnMut(&[N], &[N], &mut [N]),
    {
        let self_chunks: ChunksPad<'_, N, C> = ChunksPad::new(&self.data);
        let other_chunks: ChunksPad<'_, N, C> = ChunksPad::new(&other.data);
        let mut res = unsafe { Multiset2::new_uninitialized() };
        let mut res_chunks: ChunksPadMut<'_, N, C> = ChunksPadMut::new(&mut res.data);
    ...
    #[inline]
    fn _zip_map_chunks_exact<F, const C: usize>(&self, other: &Self, mut f: F) -> Self
        where
            N: SmallNumConsts,
            F: FnMut(&[N], &[N], &mut [N]),
    {
        let self_chunks = self.data.chunks(C);
        let other_chunks = other.data.chunks(C);
        let mut res = unsafe { Multiset2::new_uninitialized() };
        let res_chunks = res.data.chunks_mut(C);

        res_chunks.zip(self_chunks.zip(other_chunks)).for_each(|(r, (a, b))| f(a, b, r));
        res
    }
    ...
    {
        if SIZE % C == 0 {
            self._zip_map_chunks_exact::<F, C>(other, f)
        } else {
            self._zip_map_chunks_remainder::<F, C>(other, f)
        }
    }
    ...
    {
        if SIZE % C == 0 {
            self.data.chunks(C).zip(other.data.chunks(C)).all(|(a, b)| f(a, b))
        } else {
            let self_chunks: ChunksPad<'_, N, C> = ChunksPad::new(&self.data);
            let other_chunks: ChunksPad<'_, N, C> = ChunksPad::new(&other.data);
    ...
    #[inline]
    fn fold_chunks<Acc, F, const C: usize>(&self, init: Acc, mut f: F) -> Acc
        where
            N: SmallNumConsts,
            F: FnMut(Acc, &[N]) -> Acc,
    {
        let mut res = init;
        if SIZE % C == 0 {
            for slice in self.data.chunks(C) {
                res = f(res, slice);
            }
        } else {
            let self_chunks: ChunksPad<'_, N, C> = ChunksPad::new(&self.data);
    ...
    {
        if SIZE % C == 0 {
            self.data.chunks(C).all(|slice| f(slice))
        } else {
            let self_chunks: ChunksPad<'_, N, C> = ChunksPad::new(&self.data);
    ...
    {
        if SIZE % C == 0 {
            self.data.chunks(C).any(|slice| f(slice))
        } else {
            let self_chunks: ChunksPad<'_, N, C> = ChunksPad::new(&self.data);
