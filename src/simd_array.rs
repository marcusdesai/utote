...
use packed_simd::{u32x2, f64x2, m32x2};
use rand::prelude::*;
        ...
        UInt<U, B>: ArrayLength<u32x2>
{
    ...
    pub fn contains(self, elem: usize) -> bool {
        if elem < Self::len() {
            let array_index = elem / u32x2::lanes();
            let vector_index = elem % u32x2::lanes();
    ...
    #[inline]
    pub fn min(&self) -> u32 {
        self.fold(u32x2::MAX, |acc, vec| acc.min(vec)).min_element()
    }
    ...
    pub fn choose(&mut self, elem: usize) {
        ...
        for i in 0..UInt::<U, B>::USIZE {
            ...
            if i == array_index {
                let mask = m32x2::splat(false).replace(vector_index, true);
                *data = mask.select(*data, u32x2::ZERO)
            } else {
                *data *= u32::ZERO
            }
        }
        ...
        for i in 0..UInt::<U, B>::USIZE {
            ...
            if chosen {
                *elem_vec *= u32::ZERO
                ...
                'vec_loop: for j in 0..u32x2::lanes() {
                ...
                if chosen {
                    let mask = m32x2::splat(false).replace(vector_index, true);
                    *elem_vec = mask.select(*elem_vec, u32x2::ZERO)
                } else {
                    *elem_vec *= u32::ZERO
                }
            }
    ...
    pub fn shannon_entropy(&self) -> f64 {
        ...
        -self.fold(f64x2::splat(0.0), |acc, vec| {
            ...
            acc + data.is_nan().select(f64x2::ZERO, data)
        }).sum()
    }
}
