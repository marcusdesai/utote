...
impl Multiset<u32x4, U0> {
    #[inline]
    pub const fn empty() -> Self {
        Multiset { data: u32x4::ZERO }
    }

    #[inline]
    pub const fn repeat(elem: u32) -> Self {
        Multiset { data: u32x4::splat(elem) }
    }
    ...
    {
        let mut data = u32x4::ZERO;
        let mut it = iter.into_iter();

        for i in 0..u32x4::lanes() {
    ...
    pub fn from_slice(slice: &[u32]) -> Self
    {
        ...
        Multiset { data: u32x4::from_slice_unaligned(slice) }
    }

    #[inline]
    pub const fn len() -> usize { u32x4::lanes() }
    ...
    pub fn contains(self, elem: usize) -> bool {
        elem < Self::len() && unsafe { self.data.extract_unchecked(elem) > u32::ZERO }
    }

    #[inline]
    pub unsafe fn contains_unchecked(self, elem: usize) -> bool {
        self.data.extract_unchecked(elem) > u32::ZERO
    }
    ...
    fn count_zero(&self) -> u32 {
        self.data.eq(u32x4::ZERO).bitmask().count_ones()
    }

    #[inline]
    fn count_non_zero(&self) -> u32 {
        self.data.gt(u32x4::ZERO).bitmask().count_ones()
    }

    #[inline]
    pub fn is_empty(&self) -> bool {
        self.data == u32x4::ZERO
    }
    ...
    pub fn choose(&mut self, elem: usize) {
        let mask = m32x4::splat(false).replace(elem, true);
        self.data = mask.select(self.data, u32x4::ZERO)
    }
    ...
    pub fn collision_entropy(&self) -> f64 {
        ...
        let data = f64x4::from(self.data);
        -(data / total).powf(f64x4::splat(2.0)).sum().log2()
    }
    ...
    pub fn shannon_entropy(&self) -> f64 {
        let total: f64 = From::from(self.total());
        let prob = f64x4::from(self.data) / total;
        let data = prob * prob.ln();
        -data.is_nan().select(f64x4::ZERO, data).sum()
    }
}
